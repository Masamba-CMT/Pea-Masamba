...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_SPLITTING]:
                if EMOJI_THEME is True:
                    ...
                    msg += f"\n<b></b>{get_progress_bar_string(download)} » {download.progress()}"
                    msg += f"\n<b>❖ Proc:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                    msg += f"\n<b>❖ Speed:</b> {download.speed()}"
                    msg += f"\n<b>❖ ETA:</b> {download.eta()}"
                ...
                if download.message.chat.type != 'private':
                    try:
                        ...
                        if EMOJI_THEME is True:
                            ...
                            msg += f'\n<b>❖ Source:</b> <a href="https://t.me/c/{str(download.message.chat.id)[4:]}/{download.message.message_id}">Click Here</a>'
        ...
        if EMOJI_THEME is True:
            ...
            bmsg = f"\n<b>❖ 🄼🄰🅂🄰🄼🄱🄰 🄻🅄🅃🅁🄰</b>"
            bmsg += f"\n<b>❖ RAM:</b> {virtual_memory().percent}% | <b>FREE:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
            bmsg += f"\n<b>❖ DL:</b> {get_readable_file_size(dl_speed)}/s ▼ | <b>UL:</b> {get_readable_file_size(up_speed)}/s ▲"
