...
help_string = '''
<b><a href='https://t.me/Collection_MovieTerbaik'>Pea Masamba</a></b> - Telegram MIrror-Leech Bot to Upload Your File & Link in Google Drive & Telegram
        ...
        if notifier_dict := DbManger().get_incomplete_tasks():
            for cid, data in notifier_dict.items():
                if ospath.isfile(".restartmsg"):
                    ...
                    msg = f"😎Pea Masamba Restart Bot!\n"
                    msg += f"📅Tanggal: {date}\n"
                    msg += f"⌚Jam: {time}\n"
                    msg += f"🌐Zona Waktu: {TIMEZONE}"
                else:
                    msg = f"😎Pea Masamba Restart Bot!\n"
                    msg += f"📅Tanggal: {date}\n"
                    msg += f"⌚Jam: {time}\n"
